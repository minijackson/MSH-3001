...
sondage.fr <- read.csv("sondage-fr.csv")
sondage.fr$Horodateur <- as.POSIXct(sondage.fr$Horodateur)

sondage.en <- read.csv("sondage-en.csv")
sondage.en$Horodateur <- as.POSIXct(sondage.en$Horodateur)

legendaryPie <- function(x) {
  x.table <- table(x)
  if(length(x.table) >= 3) {
    if(length(x.table) >= 9) {
      x.colors <- brewer.pal(length(x.table), "Set3")
    } else {
      x.colors <- brewer.pal(length(x.table), "Pastel1")
    }
  } else {
    x.colors <- brewer.pal(3, "Pastel1")
  }
  pie(x.table, col=x.colors)
  legend("topright", legend=x.table, fill=x.colors)
}
@

<<tranche age fr, echo=FALSE, fig.cap="Tranches d'âges des personnes ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$Tranche.d.âge)
@

<<tranche age en, echo=FALSE, fig.cap="Tranches d'âges des personnes ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$Age.bracket)
@

<<sexe fr, echo=FALSE, fig.cap="Sexe des personnes ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$Sexe)
@

<<sexe en, echo=FALSE, fig.cap="Sexe des personnes ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$Gender)
@

\section{Marketing}

<<know fr, echo=FALSE, fig.cap="Pourcentage des personnes connaissant auparavant la C4 Cactus ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$Connaissez.vous.le.modèle.C4.Cactus..)
@

<<know en, echo=FALSE, fig.cap="Pourcentage des personnes connaissant auparavant la C4 Cactus ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$Are.you.familiar.with.the.Citroën.C4.Cactus.)
@

<<means global, echo=FALSE>>=
smartWordcloud <- function(texte, lang) {
  corpus <- VCorpus(VectorSource(texte))
  # Replace "'", ";" and "/" by spaces to separate words.
  ...
  wordcloud(d$word, d$freq, colors=brewer.pal(9, "PuBu")[4:9])
}
@

<<means fr, echo=FALSE, fig.cap="Nuage de mots du moyen de prise de conscience de l'existence de la C4 Cactus des personnes ayant répondu au sondage français", fig.pos="H">>=
smartWordcloud(sondage.fr$Si.oui..par.quels.moyens.en.avez.vous.entendu.parler.., "french")
@

<<means en, echo=FALSE, fig.cap="Nuage de mots du moyen de prise de conscience de l'existence de la C4 Cactus des personnes ayant répondu au sondage anglais", fig.pos="H">>=
smartWordcloud(sondage.en$If.you.are..by.which.means.have.you.heard.about.it., "english")
@

\section{Apparence de la voiture}

<<design fr, echo=FALSE, fig.cap="Nuage de mots de l'avis du design de la C4 Cactus des personnes ayant répondu au sondage français", fig.pos="H">>=
smartWordcloud(sondage.fr$Que.pensez.vous.du.design.de.la.C4.Cactus.., "french")
@

<<design en, echo=FALSE, fig.cap="Nuage de mots de l'avis du design de la C4 Cactus des personnes ayant répondu au sondage anglais", fig.pos="H">>=
smartWordcloud(sondage.en$What.can.you.say.about.the.C4.Cactus..design.and.appearance., "english")
@

<<interior fr, echo=FALSE, fig.cap="Nuage de mots de l'avis de l'intérieur de la C4 Cactus des personnes ayant répondu au sondage français", fig.pos="H">>=
smartWordcloud(sondage.fr$L.intérieur.de.la.Cactus.et.ses.fonctionnalités.vont.à.l.essentiel..Qu.en.pensez.vous.., "french")
@

<<interior en, echo=FALSE, fig.cap="Nuage de mots de l'avis de l'intérieur de la C4 Cactus des personnes ayant répondu au sondage anglais", fig.pos="H">>=
smartWordcloud(sondage.en$The.inside.of.the.Cactus..as.well.as.its.features..gets.down.to.basics..What.do.you.think.about.it., "english")
@

\section{Opinion générale}

<<qualities global, echo=FALSE>>=
coloredBarplot <- function(x, pal) {
  x <- as.vector(do.call(c, strsplit(levels(x), ";")))
  x.table <- table(factor(x))
  x.table.mapped <- as.vector(round((x.table - min(x.table))*8/max(x.table))) + 1
  x.table.colors <- do.call(c, as.list(brewer.pal(9, pal))[x.table.mapped])

  x <- barplot(x.table, xaxt='n', col=x.table.colors)
  par(cex=.6)
  labs <- names(x.table)
  text(cex=1, x=x+.5, y=-.5+(1/max(x.table)), labs, xpd=TRUE, srt=20, pos=2)
}
@

<<qualities fr, echo=FALSE, fig.cap="Qualités de la C4 Cactus selon les personnes ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
coloredBarplot(sondage.fr$Quelles.sont.selon.vous.les.qualités.de.la.Cactus.., "YlGn")
@

<<qualities en, echo=FALSE, fig.cap="Qualités de la C4 Cactus selon les personnes ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
coloredBarplot(sondage.en$In.your.opinion..what.are.the.Cactus..qualities., "YlGn")
@

<<flaws fr, echo=FALSE, fig.cap="Défauts de la C4 Cactus selon les personnes ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
coloredBarplot(sondage.fr$Quels.sont.alors.ses.défauts.., "YlOrRd")
@

<<flaws en, echo=FALSE, fig.cap="Défauts de la C4 Cactus selon les personnes ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
coloredBarplot(sondage.en$Then.what.are.its.flaws., "YlOrRd")
@

<<buy fr, echo=FALSE, fig.cap="Proportion personnes ayant répondu au sondage français qui achèteraient une C4 Cactus", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$Si.vous.deviez.acheter.une.voiture..envisageriez.vous.de.choisir.une.Cactus..)
@

<<buy en, echo=FALSE, fig.cap="Proportion personnes ayant répondu au sondage anglais qui achèteraient une C4 Cactus", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$If.you.had.to.buy.a.car..would.you.consider.buying.a.Cactus.)
@

\section{La marque DS}

<<ds know fr, echo=FALSE, fig.cap="Proportion personnes ayant répondu au sondage français sachant que DS était une marque indépendante", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$DS.est.depuis.2014.une.marque.indépendante.de.Citroën...le.saviez.vous..)
@

<<ds know en, echo=FALSE, fig.cap="Proportion personnes ayant répondu au sondage anglais sachant que DS était une marque indépendante", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$DS.left.Citroën.in.2014.to.become.an.independent.division.of.PSA..Did.you.know.about.this.)
@

<<brand fr, echo=FALSE, fig.cap="Nuage de mots de l'avis de la marque DS des personnes ayant répondu au sondage français", fig.pos="H">>=
smartWordcloud(sondage.fr$Quel.est.votre.avis.sur.la.marque.DS.., "french")
@

<<brand en, echo=FALSE, fig.cap="Nuage de mots de l'avis de la marque DS des personnes ayant répondu au sondage anglais", fig.pos="H">>=
smartWordcloud(sondage.en$What.is.your.general.opinion.about.the.brand., "english")
@

<<preference fr, echo=FALSE, fig.cap="Préférences de marque des personnes ayant répondu au sondage français", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.fr$Parmi.ces.trois.marques..laquelle.a.votre.préférence..)
@

<<preference en, echo=FALSE, fig.cap="Préférences de marque des personnes ayant répondu au sondage anglais", fig.pos="H", fig.height=5>>=
legendaryPie(sondage.en$Among.those.three.brands..current.products..whose.do.you.prefer.)
@

<<differences fr, echo=FALSE, fig.cap="Nuage de mots de l'avis de la marque DS des personnes ayant répondu au sondage français", fig.pos="H">>=
smartWordcloud(sondage.fr$Pour.vous..sur.quels.points.la.ligne.DS.se.démarque.t.elle.de.Citroën.., "french")
@

<<differences en, echo=FALSE, fig.cap="Nuage de mots de l'avis de la marque DS des personnes ayant répondu au sondage anglais", fig.pos="H">>=
smartWordcloud(sondage.en$According.to.you..what.makes.DS.different.from.Citroën., "english")
@
